...
这台机器上 `time.Now()` 的推进量子远粗于常见机器：连续采 200 万次只有 **34 个不同的非零 delta**，最小非零 delta **303 µs**（`for time.Now() == t0 {}` 等到的下一跳 642 µs）。探针是个 20 行的 Go 程序（未入库，与表 C 的采集工程同理）。
...
### 表 C：与 gin 的真实负载对比（2026-09-16 重跑，采集工程在 `bench/gin-compare`）

设计验收标准『真实负载与 gin 同级』条的落地。**结论留在这里，采集工程没有进仓库**——`loadtest/`（带 gin 依赖的独立
module）是为回答「与 gin 同级吗、观测的钱花在哪」做的验证工程，保留在分支 `bench/gin-compare`（未合并、未删除），要复现
就切过去；本页只留口径与结论。

本节 2026-09-14 首采、2026-09-16 **按同一口径重跑**：首采那轮观测档用的是**当时的**默认出口 `SlogSink`，默认换成
`ConsoleSink`（#21）之后那一档就不再代表现状（#68）。重跑顺带把采集工程里写死的出口形态改成「跟当前默认走」——
原来的写法在默认换代后会**静默地继续量旧默认**，这正是这一档需要重跑的原因。
...
- **裸档同级**：0.90–0.95×；并发 256 时 p99 反而更低（17.74ms vs 20.78ms）。
- **绝对值只做量级参考**——同一格换一次会话就能从 49k 变到 78k（±20%），所以本表只认同轮配对比值：
  与表 A / 表 B 的「ns 跨轮漂 2–4×」是同一条纪律。
- **换默认出口的效果**（只与 2026-09-14 那轮比**比值**，不比绝对值）：`obs` 档 0.65x → **0.81x**（c=64）、
  0.88x → **0.92x**（c=256）。方向一致、幅度收窄——省下的是「把 Record 摊平成 `[]any` 再交给 slog」那一步。

**观测开销拆解**（诊断档，只跑 pulse-web 一侧——它们回答「钱花在哪」，不回答「谁快」）：

| 并发 | bare | 关访问日志 | 默认 `ConsoleSink` |
|---|---|---|---|
| 64 | 55910 | 50991 (−9%) | **47226 (−16%)** |
| 256 | 64423 | 63419 (−2%) | **61098 (−5%)** |

- **记录这条路本身很便宜**：关掉访问日志后掉 9%（c=64）/ 2%（c=256）——TraceID 生成与记录组装就在这个量级里。
- **出口那笔账已经收掉大半**：把同表内「默认出口」与「关访问日志」两格相减，出口本身从上一轮的
  ~29% / ~10% 收到 **~7% / ~3%**（c=64 / c=256；两轮各自在自己的表内相减，不相减跨轮绝对值）。
  这是 #21 换默认出口的直接兑现。
- **参照**：gin 侧同一档（`gin.Default()` vs `gin.New()`，`Logger` 就是一行文本、没有第二条路可选）掉
  **6.6% / 2.1%**（c=64 / c=256，同一张主表内两行相减）。
- ⚠️ **本表去掉了三列**（`换 LineSink` / `AsyncSink(LineSink)` / 原型 `fastsink`）。原因不是数字难看，是
  **它们量的不是今天存在的选项**：`fastsink` 是 `ConsoleSink` **之前**的原型出口，它的手法已经被 #21 兑现成
  `ConsoleSink` 本身，留着等于拿一个不会发布的实现当选项去比；而那一列的「异步」包的是 `LineSink`，**不是现在的
  默认出口**。真实可选的就是「默认」与「默认外面再套一层 `AsyncSink`」，要出这一对得先按当前选项重做采集工程
  再重跑（[#73](https://github.com/Luo-root/pulse-web/issues/73)），本页不用旧组合凑数。
...
| pulse/obs-line（换 `LineSink`） | ~1425 | 1330 | 17 |
| pulse/obs-async（`AsyncSink` 包的是 `LineSink`） | ~1450 | 1650 | 18 |
| pulse/obs（**当时的**默认出口 `SlogSink`，见下） | ~2850 | 2538 | 35 |

> 本表整张来自 2026-09-14 那一轮（口径与表 A / 表 B 不同）。**那之后发生两件事**：默认出口换成 `ConsoleSink`，
> 而表里原来还有一行 `pulse/obs-fast`——它是 `ConsoleSink` **之前**的原型出口，手法已被 #21 兑现成 `ConsoleSink`
> 本身，拿它当选项比较没有意义，**此行已删**。`pulse/obs` 这一档的现状不在这张表里，它在「默认出口」一节
> （同一条请求路径：**2335 ns / 22 allocs / 6298 B/op**，与 `nopSink` 档逐项相同）。
>
> 这两张表**只能整表替换、不能只换一格**（跨轮数字相减会给出倒挂结论，本项目为此专门立过规矩），所以本表整张
> 等下一次复跑时一并刷新；那一次要把选项集收敛成今天真实存在的那几个——**默认 `ConsoleSink`** 与
> **默认外面再套一层 `AsyncSink`**（[#73](https://github.com/Luo-root/pulse-web/issues/73)）。
