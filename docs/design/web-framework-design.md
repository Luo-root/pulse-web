...
**明确不做五条**：不搬动路由（预检 `OPTIONS` 到不了中间件，只注册 `GET /api` 时 ServeMux 直接 405 `Allow: GET, HEAD`；**没匹配到路由的请求同样不经洋葱**）→ CORS 类必须外包；不解决「收尾型中间件 × error/panic」（框架错误映射发生在中间件返回**之后**）→ 压缩类推外包；不透出 `Pusher` / `FlushError`（能力面只到 `Flush` / `Hijack` / `SetWriteDeadline` / `EnableFullDuplex` 四个；`Hijacker` 会透出，`Adapt` 的代理同样转发）；不保证「同形状就能接」（chi `CleanPath` 读 `chi.RouteContext`，经 `Adapt` 与外包**都 panic**）；不改中间件语义（panic 谁接、错误响应体长什么样仍归中间件）。
