...
**明确不做五条**：不搬动路由（预检 `OPTIONS` 到不了中间件，只注册 `GET /api` 时 ServeMux 直接 405 `Allow: GET, HEAD`；**没匹配到路由的请求同样不经洋葱**，**需要归一化的路径**——`//users`、`/users//42`——则由 ServeMux 在调用 handler 之前就 307 到干净路径）→ CORS 类要么外包，要么在洋葱内加一条兜底 `OPTIONS /{path...}` 路由（后者预检照常进观测，代价是记录里的 `http.route` 记成兜底模式）；不解决「收尾型中间件 × error/panic」（框架错误映射发生在中间件返回**之后**）→ 压缩类推外包；不透出 `Pusher` / `FlushError`（能力面只到 `Flush` / `Hijack` / `SetWriteDeadline` / `EnableFullDuplex` 四个；`Hijacker` 会透出，`Adapt` 的代理同样转发）；不保证「同形状就能接」（chi `CleanPath` 读 `chi.RouteContext`，经 `Adapt` 与外包**都 panic**）；不改中间件语义（panic 谁接、错误响应体长什么样仍归中间件）。
...
| 项 | 去路 |
|---|---|
| 自研中间件实现（CORS / CSRF / 鉴权 / 限流…） | 不做——框架只给**缝**：`Adapt` 把生态里 stdlib 形状的中间件接进洋葱、`Wrap` 接 stdlib handler。CORS 要拦预检有两条路：外包 `Handler()`，或洋葱内加一条兜底 `OPTIONS /{path...}` 路由（后者预检照常进观测，见「中间件与 stdlib 互操作」）。哪些生态件实测可吸纳、各挂哪个挂载点、CORS 与 CSRF 的完整接法，见站点「stdlib 中间件接入」；**不出官方件**这一条在 [#61](https://github.com/Luo-root/pulse-web/issues/61) 定下（判据：三类候选生态件都是零依赖或近零依赖，生态件解决不了的那条缝有上面的兜底路由兜住） |
