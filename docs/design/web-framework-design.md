...
v0.2.0 发版按同一口径第四次重采**（`bare` 0.94x / 0.98x、`obs` 0.90x / 0.99x），全部落在同一
波动带，下面登的是这一轮。
...
- **裸档同级**：0.94–0.98×；c=256 的 p99 更低（20.03ms vs 24.43ms），c=64 两侧持平（4.89ms vs 4.81ms）。
...
- **换默认出口的效果**（只与 2026-09-14 那轮比**比值**，不比绝对值）：`obs` 档 0.65x → **0.82x**（c=64）、
  ...
  本轮已是 0.90x / 0.99x。省下的是「把 Record 摊平成 `[]any` 再交给 slog」那一步。

**观测开销拆解**（诊断档，只跑 pulse-web 一侧——它们回答「钱花在哪」，不回答「谁快」）：

| 并发 | bare | 关访问日志 | 默认 `ConsoleSink` | `AsyncSink`(默认出口) |
|---|---|---|---|---|
| 64 | 55579 | 53105 (−4%) | **51841 (−7%)** | 53824 (−3%) |
| 256 | 57919 | 57197 (−1%) | **58336 (+1%)** | 57454 (−1%) |

- **默认装配的观测开销**：c=64 掉 7%（关掉访问日志掉 4%）——**记录这条路本身**（TraceID 生成 +
  记录组装）的量级就在这儿；**c=256 三档全部落在 ±1% 内**（该格轮间极差 3.0%），开销被噪声淹没，
  这一格读不出差异。
- **「出口」与「记录框架」的分界落在噪声里，本页不给单独归因。** 把同表内「默认出口」与「关访问日志」
  两格相减得 3%（c=64）/ 2%（c=256）；往前数几轮同一算法给的是 4% / 3%、3% / 1%、4% / 1%、7% / 3%。
  各轮算出来不一致、又都小于 `obs` 档自身的**轮间极差**——宁可不写，也不给一个看起来
  精确的假数。
...
- [x] **真实负载与 gin 同级**（不以 micro-benchmark 胜负作承诺）——**已复采六轮、结论一致**：裸档 0.94× / 0.98×、观测档 0.90× / 0.99×（最近一轮 2026-09-22 随 v0.2.0 发版；此前 2026-09-20 放开 `Hijack` 那轮是 0.90× / 0.94×、0.87× / 0.92×，评审修复 hijack 之后的写保护后复跑 0.89× / 0.95×、0.85× / 0.91×；见「表 C」；采集工程在 `loadtest/`，CI 覆盖它的 build / vet / test）
