...
| 并发 | bare | 关访问日志 | 默认 `ConsoleSink` | 换 `LineSink` | `AsyncSink(LineSink)` | 原型 `fastsink` |
|---|---|---|---|---|---|---|
| 64 | 55910 | 50991 (−9%) | **47226 (−16%)** | 47602 (−15%) | 52203 (−7%) | 45565 (−19%) |
| 256 | 64423 | 63419 (−2%) | **61098 (−5%)** | 61891 (−4%) | 62205 (−3%) | 61714 (−4%) |
...
- **出口那笔账已经收掉大半**：把同表内「默认出口」与「关访问日志」两格相减，出口本身从上一轮的
  ...
  这是 #21 换默认出口的直接兑现。
- **换出口在真实负载下已经分不出胜负**：`ConsoleSink` / `LineSink` / `AsyncSink` / 原型 `fastsink`
  四档在 c=256 落在 61098–62205（跨度 1.8%），c=64 落在 45565–52203 且**原型反而最慢**——而这几档的
  逐轮极差就有 4–8%，**本表不为它们排序**。这本身是结论：默认值换对之后，「出口选型」不再是观测档
  掉分的主因；要再往下抠得看请求路径，不是出口。
...
> 本表整张来自 2026-09-14 那一轮（口径与表 A / 表 B 不同）。默认出口换成 `ConsoleSink` 之后 `pulse/obs`
> 这一档的现状**不在这里**——它在「默认出口」一节（同一条请求路径：**2335 ns / 22 allocs / 6298 B/op**，
> 与 `nopSink` 档逐项相同）。这两张表**只能整表替换、不能只换一格**（跨轮数字相减会给出倒挂结论，本项目
> 为此专门立过规矩），所以本表等下一次整体复跑时一并刷新——那是一次独立的采集，不塞进本票。
