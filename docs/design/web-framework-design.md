...
迁移带来**两处口径变化**（其余 42 项逐字节不变，44 组对照见 #27）：

1. **耗时列**：浮点四舍五入 → 上游 `AppendDuration` 的整数截断（`585199ns` 旧 `585.2µs` → 新 `585.1µs`）
2. **列补齐**：`utf8.RuneCount` → `observability.DisplayWidth`。ASCII 下等价，**全角下是新版才对**——`客户端-甲:1234` 是 10 rune / 14 显示列，旧实现补 5 个空格让该列占 19 列、后续列整体推右 4 格；新版补 1 个、占满 15 列。`TestConsoleSinkPaddingUsesDisplayWidth` 钉住

两处的共同点：都是把「本地第二份实现」换成上游的一致性资产，**换的方向是收敛而不是偏好**。
