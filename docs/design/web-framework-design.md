...
**复测记录（2026-09-15，同一台机器，电池档）**：为站点性能页复跑本表口径，得到一个比表 A 更干净的对照。同轮里加跑了一条**与框架无关**的对照基准（`BenchmarkStdlibServeMux_Route`，纯标准库）：

| | 2026-09-13（插电） | 2026-09-15（电池） | 变化 |
|---|---|---|---|
| stdlib 对照行 | 174 ns | **750 ns** | ×4.3 |
| `Engine` 请求路径（`New()`） | 1790 ns | **6592 ns** | ×3.7 |
| 同上 · allocs / B/op | 22 / 6314 | **22 / 6298** | **不变** |
| `WithCollector()` 档 · allocs / B/op | 34 / 6803 | **34 / 6787** | **不变** |

同一份代码、同一台机器：**ns 整体涨 3.7–4.3 倍，分配计数与 B/op 一模一样**——电池模式下睿频受限所致。这条记录有两个用处：① 它是「分配是确定性判据、ns 不是」最直白的一次实证（不是论证，是实测）；② 它说明**任何一轮 ns 都必须带同轮对照行**才能被解读，否则读者会把机器状态当成框架性能。站点性能页据此改成「每张表带对照行 + 声明机器状态」的版式。
