...
>
> **引用口径**：本文中 `包/文件.go:NNN` 形式的行号引用针对 **pulse v0.2.5**（[#104](https://github.com/Luo-root/pulse-web/issues/104) 已逐条复核：v0.2.4 → v0.2.5 之间**零 `.go` 变化**，下述四处引用所在文件未变动，行号与符号位置逐条一致）。上游一个 patch 版本就可能让行号整体位移（v0.2.1 → v0.2.2 就让 `observability/collector.go` 位移了 11 行），所以**升依赖时必须整体复核**——用 `git grep -n '\.go:[0-9]'` 取出全部引用，逐个按符号在新版本里重新定位，不要只看 diff 触及的那几条。
...
| kernel 作用 | 每请求派生 scope（实测 84 ns / 2 allocs，表 B）；**请求级数据不占全局仓库** | pulse v0.2.1 `#169` 后服务变更按依赖名索引投递（不再 O(插件树)）；请求级值走 `kernel.Local()` 作用域局部绑定 |
...
### 表 A：pulse v0.2.0 → pulse v0.2.1 升级对照（**历史，两轮各跑**）
...
| 场景 | pulse v0.2.0 | pulse v0.2.1 | Δ |
|---|---|---|---|
...
**结论**：pulse v0.2.1（`#169`）把服务变更从「全树广播」改成「按依赖名索引投递」，每请求 Provide 的成本**与插件树规模彻底解耦**（100 插件 4946 → 394 ns）。这正是红线 2 的原始依据——依据消失，红线按新语义重写。请求级数据改用 `kernel.Local()`（作用域局部绑定：不写全局仓库、不投递变更、随作用域销毁撤除），而不是把全局 `Provide` 当请求级容器。
...
| Engine 请求路径 + 一层 `Adapt`（生态中间件入口） | —（两行同轮配对的 Δ 见下） | **27** | 6466 |
...
- **生态适配层的每请求成本 = +5 allocs / +152 B**（[#89](https://github.com/Luo-root/pulse-web/issues/89) 验收第 5 条）：default 档 22 → adapt 档 **27 allocs/op**（B/op 6314 → **6466**）。这一档 2026-09-21 采，字节列与分配列**跨轮可比**——同一次复测里 default 行仍是 6314 / 22，与表内旧值逐项相同（ns 列才是会漂的那一列，所以本行不发布 ns，只在下面给同轮配对值）。
  拆解不是推测，是 `-gcflags=-m` 的逃逸分析：① `&adaptProxy{}`（请求级状态）② 交给中间件的内层 `http.HandlerFunc` 闭包 ③④ 它捕获的 `called` / `err` **各自装箱**（`moved to heap`）⑤ 中间件自己那层包装器（pass-through 形状每次构造重建闭包；用 `return next` 的对照实测少 1，即**适配层本身是 4**）。`defer` 还原闭包不分配（`does not escape`）。**票面原先预期 +2，实测 +5**——多出来的三次是两个捕获变量的装箱与中间件自身那层。同轮配对 ns：default 1875 → adapt 1905（+30 ns，两档各 5 次离散度约 ±5%，只做量级参考）。
  其中 ③④ 是**可以省的**（搬到 `adaptProxy` 上就不必单独装箱——proxy 本来就是请求级对象）；本轮只做记账不动实现，**省不省由维护者定**（改的是请求路径，要重跑本表与表 C）。
...
### 表 C：与 gin 的真实负载对比（2026-09-22 重采，采集工程在 `loadtest/`）
...
`obs` 0.85x / 0.91x）——两次落在同一波动带；2026-09-21 因 `Adapt` 的回填修好（同样动了请求路径）
按同一口径复跑（`bare` 0.90x / 0.93x、`obs` 0.86x / 0.93x），仍在同一波动带；**2026-09-22 随
v0.2.0 发版按同一口径第四次重采**（`bare` 0.94x / 0.98x、`obs` 0.90x / 0.99x）；同日随 **v0.2.1 发版做第五次重采**
（依赖升至 pulse v0.2.5——相对 v0.2.4 **零 `.go` 变化**，本仓库代码同样未动，两侧跑的是逐字节相同的代码——
`bare` 0.92x / 0.97x、`obs` 0.89x / 0.96x）。全部落在同一波动带，下面登的是这一轮；比上一轮略低**不是回归**，
前后两轮之间只差机器状态。
...
- **裸档同级**：0.92–0.97×；c=256 的 p99 更低（20.26ms vs 24.10ms），c=64 两侧持平（4.97ms vs 4.88ms）。
- **绝对值只做量级参考**——同一格换一次会话就能从 28k 变到 78k（2026-09-20 那轮的绝对 RPS 只有本轮
  的一半左右，纯属机器状态），所以本表只认同轮配对比值：与表 A / 表 B 的「ns 跨轮漂 2–4×」是同一条纪律。
- **换默认出口的效果**（只与 2026-09-14 那轮比**比值**，不比绝对值）：`obs` 档 0.65x → **0.82x**（c=64）、
  0.88x → **0.93x**（c=256）——那是换出口当轮（2026-09-16）的读数；此后各轮在同一波动带内缓慢上移，
  本轮已是 0.89x / 0.96x。省下的是「把 Record 摊平成 `[]any` 再交给 slog」那一步。

**观测开销拆解**（诊断档，只跑 pulse-web 一侧——它们回答「钱花在哪」，不回答「谁快」）：

| 并发 | bare | 关访问日志 | 默认 `ConsoleSink` | `AsyncSink`(默认出口) |
|---|---|---|---|---|
| 64 | 54411 | 52322 (−4%) | **49743 (−9%)** | 52460 (−4%) |
| 256 | 57097 | 56569 (−1%) | **56605 (−1%)** | 57103 (+0%) |

- **默认装配的观测开销**：c=64 掉 9%（关掉访问日志掉 4%）——**记录这条路本身**（TraceID 生成 +
  记录组装）的量级就在这儿；**c=256 三档全部落在 ±1% 内**（该格轮间极差 2.7%），开销被噪声淹没，
  这一格读不出差异。
- **「出口」与「记录框架」的分界落在噪声里，本页不给单独归因。** 把同表内「默认出口」与「关访问日志」
  两格相减得 5%（c=64）/ 0%（c=256）；往前数几轮同一算法给的是 3% / 2%、4% / 3%、3% / 1%、
  4% / 1%、7% / 3%。同一算法逐轮从 0% 摆到 7%、方向也不定（本轮 c=256 直接归零）——宁可不写，
  也不给一个看起来精确的假数。
- **异步出口的倾向继续减弱**：`AsyncSink(默认)` 在 c=64 是四档里最接近 `bare` 的（−3%）；c=256 四档
  全部落在 ±1% 内、**无法区分**。低并发下把格式化挪出请求路径划算、高并发下队列与调度把那点收益
  吃掉——这条倾向在减弱，仍是**倾向**，不是结论。
- **参照**：gin 侧同一档（`gin.Default()` vs `gin.New()`，`Logger` 就是一行文本、没有第二条路可选）
  掉 **4.6% / −1.0%**（c=64 / c=256，同一张主表内两行相减）；c=256 那格符号翻转——gin 的 `obs` 行比它的 `bare` 行还高 1%，
  即该格读不出中间件成本。
...
1. 请求路径一律 `EmitLocal`，禁用全树 `Emit`（pulse v0.2.1 实测 23ns vs 1778ns）
...
**两个动作**：① 把 CORS 中间件压进洋葱；② 从那时起**每次注册方法路由都顺带给同一路径补一条 `OPTIONS`**。第二条是「路由匹配先于中间件」这条边界在 CORS 上的落点：只注册 `GET /api` 时 `OPTIONS /api` 被 ServeMux 直接 405 掉，预检根本到不了中间件。`register` 与 `handle` 拆开，是为了让补出来的 `OPTIONS` 走同一条装配路径、又不再次触发观察逻辑（否则递归）。

**缝收在包内 `routeObserver`**（[#101](https://github.com/Luo-root/pulse-web/pull/101) 评审）：第一版在 `register` 里写死 `if e.cors != nil { e.cors.observe(...) }`——那等于在引擎上挖了一条 CORS 形的缝，下一个「要钩注册的一方件」（CSRF 要跳过安全方法的 `OPTIONS`、WebDAV 要补方法）还得回引擎加字段、改这两行。所以本 PR 把它收成一个**不导出**的 `routeObserver` 列表：`observe(e *Engine, pattern string)`，要补伴生路由就自己调 `e.handle`。三条边界写死在这里：

- **不导出、不做插件目录**。公开 `OnRegister(func)` 等于给「装配期任意改写路由表」发许可证，而这里被证明需要的只有「一方件能补自己的伴生路由」一件事。同理不做「所有引擎默认补 `OPTIONS`」——那会破坏「不调 `CORS` 行为逐字节一致」。
- **签名里没有 per-route `mw`**。伴生路由的链取 `e.mw`（全局 + 本节点分组），不取触发它的那条业务路由的 per-route 中间件——`OPTIONS` 不是 `GET`/`POST` 的替身。第一版把 `mw` 递进去并用了它，后果是 `GET /x` 上的鉴权守卫会连 `OPTIONS /x` 一起挡掉；不给这个参数，第一个消费者就拿不错（`TestCORSAutoOptionsKeepsEngineChainOnly`）。
- **按节点生效**，与 `mw` 同款：`Group` 时按值拷贝，slice 本身也要复制（共享 backing array 时一边 `append` 会悄悄改掉另一边已登记的元素，不报错、只有一方件静默失效）——`TestGroupCopiesObservers` 白盒钉住。「先 `Group` 再挂」与「先 `Group` 再 `Use`」同型：不生效。
...
**同一轮实测还挖出第二条更硬的理由**：兜底 `OPTIONS /{path...}` 与 `Static` 挂的 `"<prefix>/"` 在 ServeMux 上**互不更具体**（前者匹配的方法更多、后者的路径更具体），两条都注册就是**装配期 panic**（`pattern "/assets/" ... conflicts with pattern "OPTIONS /{path..."；两个注册顺序都一样）。也就是说兜底那条写法**不能与静态目录共存**——按路径逐条补同时满足这两条。这条顺带修掉了存量配方里的一个坑：站点上「`rs/cors` + 兜底 `OPTIONS /{path...}`」那条建议同样会与 `Static` 冲突，站点两页已加警示与替代选择。

**挂载口径（一个 mux 一份白名单）**：整站一份 = 先 `app.CORS` 再 `Group` / 注册路由；只覆盖某个前缀 = **只**在那个分组上挂、父上不挂；已经挂过 CORS 的节点（含从它派生的分组）再调是**装配期 panic**——父子共享同一个 mux，两份策略只会打架（`TestCORSOnCorsNodePanics`、`TestCORSAfterGroupIsNotSeenByGroup`）。顺序建议：`CORS(...)` 写在其它 `Use` **之前**（它自己就是一次 `Use`，谁在前谁在外层，想让预检跳过鉴权就先挂它）。
...
- 请求头**默认表 = `Accept` / `Content-Type`**（追加式），不是空表：`application/json` **不在** CORS 安全列表里（安全列表只认 form-urlencoded / multipart / text/plain），浏览器对 JSON POST 会带 `Access-Control-Request-Headers: content-type` 发预检——默认空表的后果是「只配了来源的 JSON API，预检全 403」，与「一方件、挂上就用」直接冲突（[#101](https://github.com/Luo-root/pulse-web/pull/101) 评审点名的第一条）。默认面因此与 `rs/cors` 对齐（`TestCORSAllowHeadersDefault`）。方法默认 `GET` / `POST` / `HEAD`，且 `CORSAllowMethods` 是**覆盖**（默认表非空，只有覆盖才收得窄）而来源 / 请求头是**追加**——这处不对称写进了各自的 godoc。
...
口径 `-benchtime=20000x -count=10` 取中位轮（2026-09-16；原表 `~190 ns` / `1531 ns / 19 allocs` / `~262 ns / 1 alloc` 是 pulse v0.2.4 之前的会话，**只能整表替换**，不能只换一格）。请求路径那一列对照的是 `nopSink` 档（1963 ns / 22 allocs / 6315 B/op，`bench/budget_test.go` 的 `default+console-sink` 一档是它的门禁）：**分配计数与 B/op 都逐项相同**——出口渲染既不新增分配、也不改变请求路径的分配形状；ns 上多出的那一段就是渲染（2335 − 1963 = 372 ns）。
...
| 公开的路由钩子（`OnRegister(func)` / 观察者接口 / 插件目录） | 不做——缝**收在包内**：`routeObserver`（不导出）只服务框架自己那些「挂在路由树上、又要参与路由注册」的一方件（CORS 是第一个消费者）。公开它等于给「装配期任意改写路由表」发许可证，而真正被证明需要的只有「一方件能补自己的伴生路由」一件事。要扩展行为请走既有三条缝：中间件（`Use` / `Group`）、`Wrap`、`Adapt` |
...
| OpenAPI / Swagger 的 spec 生成器 | 不做——handler 是 `func(*Ctx) error`（**零类型信息**），路由直接进 stdlib `ServeMux`（引擎不保留路由表，且 `ServeMux` 没有枚举 API），要在框架里做就得新增一整套声明词汇表，与「零依赖 / 不分领域语义 / 不允许逃生舱」三条同时冲突，而那套词汇表注定是 OpenAPI 的劣化副本。生态里只有把 OpenAPI 当一等公民的框架自带（huma / Fuego / goa），通用路由器一律交给外部包；接法（注释驱动 / spec 优先 / huma 叠坐）与代价（子树挂载后 `http.route` 变粗）见站点「OpenAPI 工具链」页（#62、#87） |
...
├── interop/                   # 生态互操作对照（**独立 module**，带 websocket / chi / prometheus 等真依赖）
│                              #   middleware_test.go = 中间件对照矩阵；websocket_test.go = 两条升级端到端
...
- [x] **CORS 自带且预检进观测**（[#61](https://github.com/Luo-root/pulse-web/issues/61)）：`app.CORS(...)` 零第三方依赖（主模块依赖图不变，见「设计红线」的判据）；按已注册路径补 `OPTIONS`——预检拿到 `204` + CORS 头**且进访问记录与 Trace**（`http.route` 是真路由）；被拒的预检是 403 + 统一错误体（`error.type` 可见）；`CORSAllowOrigins("*")` 与 `CORSAllowCredentials()` 装配期 panic；**不注册兜底 `OPTIONS /{path...}`**——未匹配请求照旧 404
  证据：`cors_test.go` 22 条——预检允许与三类拒绝（`TestCORSPreflightAllowed` / `TestCORSPreflightDenied`）、实际请求三态（`TestCORSActualRequest`）、通配来源（`TestCORSWildcardOrigin`）、**默认请求头表跑得动 JSON**（`TestCORSAllowHeadersDefault`）、裸 `OPTIONS` 与 `Allow` 现算（`TestCORSBareOptions` / `TestCORSBareOptionsAllowListsEveryMethod`）、通配路由与分组（`TestCORSPreflightOnWildcardRoute` / `TestCORSOnGroup`）、**挂载口径两条**（`TestCORSAfterGroupIsNotSeenByGroup`、`TestCORSOnCorsNodePanics`）、**伴生 `OPTIONS` 的链只取 `e.mw`**（`TestCORSAutoOptionsKeepsEngineChainOnly`）、`Static` 前缀（`TestCORSPreflightOnStaticPrefix`）、自定义 `OPTIONS` 优先与晚注册 panic（`TestCORSSpecificOptionsRouteWins` / `TestCORSUserOptionsAfterRoutePanics`）、重复挂载 panic（`TestCORSRegisteredTwicePanics`）、请求头列表脏形态容错（`TestCORSHeaderListTolerance`）、默认方法（`TestCORSMethodDefault`）、来源大小写与尾斜杠（`TestCORSOriginTrailingSlashAndCase`）、**两条边界**（`TestCORSUnmatchedRouteHasNoCORSHeaders`：未匹配仍 404 且不带 CORS 头——这条同时是「没退回兜底路由」的回归守卫 / `TestCORSPreflightOnUnregisteredPathIs404`）、装配期 panic（`TestCORSAssemblyPanics`）；缝本身另有一条白盒守卫 `TestGroupCopiesObservers`（`engine_test.go`：父子不共享观察者 slice 的 backing array）。**变异探针全部以「目标用例真变红」抓到**（裸 `OPTIONS` 改答 200 / 预检不校验来源 / `Allow-Methods` 回全量 / 来源不规范化 / 实际请求被拒仍加头 / 装配期不拦「无来源」/ **退回兜底 `OPTIONS /{path...}`**——这条一次性点红 11 条用例 / **伴生链混进 per-route 守卫** / **默认头表退回空** / **去掉重复挂载那道门** / **`Group` 不再拷贝观察者表** / 两版站点删掉 `CORSOption` / interop 配方不挂兜底路由），无 BUILD-ONLY。
...
- [x] **真实负载与 gin 同级**（不以 micro-benchmark 胜负作承诺）——**已复采七轮、结论一致**：裸档 0.92× / 0.97×、观测档 0.89× / 0.96×（最近一轮 2026-09-22 随 v0.2.1 发版——依赖升至 pulse v0.2.5，两侧代码逐字节未变；同日随 v0.2.0 发版那轮是 0.94× / 0.98×、0.90× / 0.99×；此前 2026-09-20 放开 `Hijack` 那轮是 0.90× / 0.94×、0.87× / 0.92×，评审修复 hijack 之后的写保护后复跑 0.89× / 0.95×、0.85× / 0.91×；见「表 C」；采集工程在 `loadtest/`，CI 覆盖它的 build / vet / test）
