...
// 「钱花在哪」，不回答「谁快」。
//
// 出口选项**只留今天真实存在的那几个**：默认出口，以及它外面再套一层 `AsyncSink`。
// 历史上有过两个不该再出现的档：`obs-line`（换 `LineSink`——它的手法已经被默认出口
// 吸收）和 `obs-fast`（`fastsink` 原型出口，一个不会发布的实现，拿它当选项比较没有
// 意义）。诊断矩阵里出现不存在的选项，比没有结论更糟。
type Mode string
...
const (
	...
	// （Bootstrap + Trace + 访问日志）+ **默认出口**（`ConsoleSink`，给人读的
	// 列式单行；见 engine.go 的 newDefaultSink）。
	//
	...
	// ModeObsAsync：默认出口外面再套一层 AsyncSink——请求路径只做 Attrs 深拷 +
	// 入队，格式化与写出挪到后台协程。这是「默认装配 + 异步出口」的常规组合。
	//
	// 内层用 ConsoleSink 而不是缓冲出口：`AsyncSink` 只排空它自己的队列、不级联内层
	// 的 Flush，内层要是还缓冲着，进程退出时那截就丢了。
	ModeObsAsync Mode = "obs-async"

	// ModeObsNoLog 诊断：默认装配但关掉访问日志（`WithoutAccessLog()`），
	// 用来把观测开销拆成「Trace + 记录框架」与「访问日志 + 出口」两段。
	//
	// 它不是出口选项，是分解用的对照——所以它进拆解表、不进「可选出口」的清单。
...
//
// 跟着默认走、不要写死具体出口类型：本项目就因为写死了当时的默认（`SlogSink`），
// 在默认换成 ConsoleSink 之后让这一档静默地量了一轮旧默认。
...
func New(mode Mode) http.Handler {
	...
	case ModeObsAsync:
		app = web.New(web.WithSink(observability.NewAsyncSink(discardConsole())))
	case ModeObsNoLog:
