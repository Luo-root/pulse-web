...
- 本轮（2026-09-20）重跑过两次，登的是后一次：先因**放开响应写出器的 `Hijack`**（动了请求路径上的 writer）按同一口径重采一次（0.90× / 0.94×、0.87× / 0.92×），随后评审修复 hijack 之后的写保护又复跑一轮复核（0.89× / 0.95×、0.85× / 0.91×）——两次落在同一波动带，**结论未变**。2026-09-21 又因 `Adapt` 的回填修复（同样在请求路径上）复跑一轮（0.90× / 0.93×、0.86× / 0.93×），仍在同一波动带。
