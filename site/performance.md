...
**版本**：pulse-web v0.1.0（当前 `main`，首个 tag 尚未发布；依赖 pulse v0.2.4）。打 tag 时这一节随该版本重测一次。

**本轮**：2026-09-16 · i9-14900HX / 32 逻辑核 / Go 1.27 / Windows amd64 · **插电**。
...
- **裸档同级**：0.91–0.95×；并发 256 时 p99 反而更低（17.43ms vs 21.30ms）。对拍档是 `gin.New()` ↔ `web.New(web.Minimal())`——两侧都没有默认中间件。
- **观测档**（`gin.Default()` ↔ `web.New()`，两侧都开默认中间件）：0.82× / 0.93×。pulse-web 这一档比 gin 多做三件事——TraceID、路由模板（`/users/{id}` 而不是实际路径）、错误分类；作为参照，gin 自己的默认中间件在这两档之间的代价是 2.7% / 3.1%。
- **口径的效力在「配对」上，不在绝对值上**：同一格换一次会话能从 49k 变到 78k，所以只认同轮配对比值（供电状态这类整机漂移被配对抵消）。**两次独立会话的配对比值逐项吻合**（0.90/0.91、0.95/0.95、0.81/0.82、0.92/0.93）——这才是它比绝对值可信的地方。
- 上一轮（2026-09-14）观测档是 **0.65× / 0.88×**——那轮用的是**当时的**默认出口 `SlogSink`；默认换成 `ConsoleSink` 后按同一口径重采，就是上面这两个数。绝对值跨轮不可比，这里只比同轮配对出来的比值。
