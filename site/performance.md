...
**版本**：pulse-web 的 `main`（v0.1.0 之后尚未再打 tag；依赖 pulse v0.2.4）。**换默认出口、动请求路径、打 tag 时都重测这一节**（下一次随 v0.2.0 发布重测），并且只比同轮配对出来的比值。

**本轮**：2026-09-20 · i9-14900HX / 32 逻辑核 / Go 1.27 / Windows amd64 · **插电**。
...
- **裸档同级**：0.89–0.95×；两个并发档的 p99 都更低（11.32ms vs 11.60ms、31.18ms vs 37.99ms）。对拍档是 `gin.New()` ↔ `web.New(web.Minimal())`——两侧都没有默认中间件。
- **观测档**（`gin.Default()` ↔ `web.New()`，两侧都开默认中间件）：0.85× / 0.91×。pulse-web 这一档比 gin 多做三件事——TraceID、路由模板（`/users/{id}` 而不是实际路径）、错误分类；作为参照，gin 自己的默认中间件在这两档之间的代价是 4.6% / 2.8%。
- **口径的效力在「配对」上，不在绝对值上**：同一格换一次会话能从 28k 变到 78k（这一轮的绝对 RPS 只有上一轮的一半左右，纯属机器状态），所以只认同轮配对比值（供电状态这类整机漂移被配对抵消）。**早先两次独立会话的配对比值逐项吻合**（0.90/0.91、0.95/0.95、0.81/0.82、0.92/0.93）——这才是它比绝对值可信的地方。
- 上一轮（2026-09-14）观测档是 **0.65× / 0.88×**——那轮用的是**当时的**默认出口 `SlogSink`；默认换成 `ConsoleSink` 后按同一口径重采，就是 0.82× / 0.93×。绝对值跨轮不可比，这里只比同轮配对出来的比值。
- 本轮（2026-09-20）重跑过两次，登的是后一次：先因**放开响应写出器的 `Hijack`**（动了请求路径上的 writer）按同一口径重采一次（0.90× / 0.94×、0.87× / 0.92×），随后评审修复 hijack 之后的写保护又复跑一轮复核（0.89× / 0.95×、0.85× / 0.91×）——两次落在同一波动带，**结论未变**。
