...
- **裸档同级**：0.90–0.94×；两个并发档的 p99 都更低（11.27ms vs 11.45ms、32.42ms vs 38.89ms）。对拍档是 `gin.New()` ↔ `web.New(web.Minimal())`——两侧都没有默认中间件。
- **观测档**（`gin.Default()` ↔ `web.New()`，两侧都开默认中间件）：0.87× / 0.92×。pulse-web 这一档比 gin 多做三件事——TraceID、路由模板（`/users/{id}` 而不是实际路径）、错误分类；作为参照，gin 自己的默认中间件在这两档之间的代价是 5.7% / 2.7%。
...
- 本轮（2026-09-20）重跑的原因是**放开了响应写出器的 `Hijack`**（动了请求路径上的 writer），按同一条纪律重采一次；结论未变。
