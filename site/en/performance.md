...
- **Per-request cost of bringing one ecosystem middleware in through `Adapt`** = **+5 allocs / +152 B** (22 → 27, 6314 → 6466 — a **deterministic** criterion, pinned by the `default+adapt` case of the allocation gate). Breakdown in Table B of the design doc: the adapter itself accounts for 4 (`adaptProxy`, the inner closure, and two boxed captures `called` / `err`), the middleware's own wrapper for 1; **two of those are avoidable**, and this round only accounts for them rather than changing the implementation.
...
## Real load against gin (2026-09-22, on AC power)
